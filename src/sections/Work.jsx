      ...
      <div ref={workRef} className="min-h-screen bg-white text-black py-24 lg:py-40 overflow-hidden">
        ...
        <div className='main-container pb-8 lg:pb-12 flex max-md:flex-col gap-6 justify-between items-start md:items-end'>
          <div className='max-w-xl'>
            <h3 className='mb-3'>Trabajo Seleccionado</h3>
            <p className='text-lg lg:text-xl'>Una muestra de mis proyectos seleccionados, diseñados para inspirar, involucrar y ofrecer resultados reales.</p>
          </div>
          <GradientButton text="Explora TODO" link="/projects" className="btn-light" />
        </div>
          <div ref={projectsRef}>
            {/* Projects */}
            <div className='flex gap-4 lg:gap-8 ms-4 lg:ms-[40%] mt-6'>
              {projects.slice(0, 3).map(({ id, name, image, link }) => (
                <a
                  key={id}
                  href={link}
